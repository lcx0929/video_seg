...
from utils.vos_datasets import YTVOSDataset
...
def parse_args(args):
    ...
    parser.add_argument(
        "--version", default="/home/lcx/projcet/lisa/LISA/model/llava-v1.5-7b"
    )
    ...
    parser.add_argument(
        "--vision-tower", default="/home/lcx/projcet/models/clip-vit-large-patch14", type=str
    )
    ...
    parser.add_argument("--vision_pretrained", default="/home/lcx/projcet/lisa/LISA/model/sam_vit_h.pth", type=str)
...
def main(args):
    ...
    if args.no_eval == False:
        if args.val_dataset=="ytvos":
            val_dataset = YTVOSDataset(
                args.dataset_dir,
                args.vision_tower,
                args.image_size,
                train=False,
...
def validate(val_loader, model_engine, epoch, writer, args):
    ...
    for input_dict in tqdm.tqdm(val_loader):
        ...
        pred_masks_image = tensor_to_image(pred_masks[0])

        # 保存图像
        pred_masks_image.save('tensor_image.png')'''
        output_list = (pred_masks[0] > 0).int()
        assert len(pred_masks) == 1
        if output_list.shape[0]==0:
            output_list = torch.zeros(masks_list.shape).cuda()
