...
class LISAForCausalLM(LlavaLlamaForCausalLM):
        ...
        **kwargs,
        ...
        if len(images.shape)==4:
            ...
            for i in range(images.shape[1]):
                image_embeddings.append(self.get_visual_embs(images[:,i,:,:,:].squeeze(1)))
        ...
        if len(images.shape)==4:
            ...
            for j in range(images_clip.shape[1]):
                ...
                for i in range(len(offset) - 1):
                    ...
                    images_clip_list.append(images_clip_i)
                images_clip_ = torch.cat(images_clip_list, dim=0)
            ...
            video_feat = self.query_feat.weight.unsqueeze(0).expand(pred_embeddings_video.shape[0],-1,-1)
            ...
            for batch_idx in range(len(pred_masks)):
                gt_mask = gt_masks[batch_idx]
                pred_mask = pred_masks = torch.stack(pred_masks)
                gt_mask_list = []
                if gt_mask.shape!= pred_mask.shape:
    ...
    def expand_masks(self, gt_masks1,gt_masks2, expand_list):
