...
class ReferSegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i,text in enumerate(sampled_classes):
            if isinstance(sampled_ann_ids[i],list):
                if sampled_ann_ids[i][0] == -1:
                    text = text.strip()
                    assert len(text.split("||")) == 1
                    question_template = random.choice(self.short_question_list)
                    questions.append(question_template.format(class_name=text.lower()))
                    rej = '[REJ]'
                    answers.append(random.choice(self.answer_list).replace('[SEG]', rej))
                else:
                    text = text.strip()
                    assert len(text.split("||")) == 1
                    question_template = random.choice(self.short_question_list)
                    questions.append(question_template.format(class_name=text.lower()))
                    cls_seg = text+' [SEG]'
                    #answers.append(random.choice(self.answer_list).replace('[SEG]', "[SEG{}]".format(i+1)))
                                ...
                                if type(ann["segmentation"]) == dict: 
                                        ...
                                        '''for i in range(len(rle)):
                                            if not isinstance(rle[i]["counts"], bytes):
                                                rle[i]["counts"] = rle[i]["counts"].encode() #''' #polygon
        ...
        '''if ds == 'grefcoco' and flag:
            ...
            save_dir = os.path.join("root/data", image_name.split(".")[0])
