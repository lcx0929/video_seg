...
class REFER:
    def __init__(self, data_root, dataset="refcoco", splitBy="unc"):
        ...
        self.DATA_DIR = osp.join(self.DATA_DIR, dataset)
        ...
        tic = time.time()
        if dataset == 'grefcoco':
            ref_file = osp.join(self.DATA_DIR, "grefs(" + splitBy + ").json")
        else:
    ...
    def createIndex(self):
        ...
        for ref in self.data["refs"]:
            # ids
            ref_id = ref["ref_id"]
            ann_id = ref["ann_id"][0] if isinstance(ref["ann_id"], list) else ref["ann_id"]
            category_id = ref["category_id"][0] if isinstance(ref["category_id"], list) else ref["category_id"]
