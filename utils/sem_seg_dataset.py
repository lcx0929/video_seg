...
def init_ade20k(base_image_dir):
    with open("/media/lcx/sia-vision/lisa/LISA/utils/ade20k_classes.json", "r") as f:
...
def init_cocostuff(base_image_dir):
    cocostuff_classes = []
    with open("/media/lcx/sia-vision/lisa/LISA/utils/cocostuff_classes.txt") as f:
...
class SemSegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for sampled_cls in sampled_classes:
            ...
            cls_seg = text.replace('_',' ')+' [SEG]'
            answers.append(random.choice(self.answer_list).replace('[SEG]',cls_seg))
        ...
        return (
            image_path,#'/root/autodl-tmp/dataset/coco/train2017/000000462461.jpg'
            image,#(3,1024,1024)
            image_clip,# ([3, 224, 224])
            conversations,#"A chat between a curious human and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the human's questions. USER: <image>\nWhat is the lead of the pencil in this image? Please output segmentation mask. ASSISTANT: Sure, the segmentation result is [SEG].</s>"
            masks,#torch.Size([3, 263, 500])
            label,#torch.Size([263, 500])
            resize,# (539, 1024)
            questions,#'<image>\nWhat is the lead of the pencil in this image? Please output segmentation mask.'
            sampled_classes,#['the lead of the pencil', 'earphone headband', 'pencil']
        )
